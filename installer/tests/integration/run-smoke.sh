...
scp "${SCP_OPTS[@]}" \
    ...
    "$INSTALLER_DIR/on-device/xovi-trigger.service" \
    "$INSTALLER_DIR/on-device/bin/xovi-trigger" \
...
log "PHASE 4: install-trigger (listener install, overlay-safe)"
$SSH "sh /tmp/rm-install-test/install-trigger.sh /tmp/rm-install-test"

assert_file_exists_on_mock /etc/systemd/system/xovi-trigger.service
assert_file_contains /etc/systemd/system/xovi-trigger.service "Wants=home.mount"
assert_file_contains /etc/systemd/system/xovi-trigger.service "ExecStart=/home/root/xovi-trigger/xovi-trigger"
assert_file_exists_on_mock /home/root/xovi-trigger/xovi-trigger
if $SSH "[ -L /etc/systemd/system/multi-user.target.wants/xovi-trigger.service ]"; then
    pass "multi-user.target.wants/xovi-trigger.service symlink created (enable)"
else
    fail "missing enable symlink at /etc/systemd/system/multi-user.target.wants/xovi-trigger.service"
fi
...
pass "xochitl.service.d untouched by install-trigger (stock invariant holds)"

log "  verifying legacy xovi-activate artefacts are NOT present"
if $SSH "! [ -f /etc/systemd/system/xovi-activate.service ]"; then
    pass "no stale xovi-activate.service after install-trigger"
else
    fail "xovi-activate.service should not exist after a clean install-trigger"
fi
...
if $SSH "[ -L /etc/systemd/system/multi-user.target.wants/xovi-trigger.service ]"; then
    ...
    fail "enable symlink did NOT survive simulated reboot, trigger persistence broken"
fi
assert_file_exists_on_mock /home/root/xovi-trigger/xovi-trigger
...
if $SSH "! [ -f /etc/systemd/system/xovi-trigger.service ]"; then
    pass "xovi-trigger.service removed"
else
    fail "xovi-trigger.service still present after uninstall"
fi
if $SSH "! [ -L /etc/systemd/system/multi-user.target.wants/xovi-trigger.service ]"; then
...
if $SSH "! [ -d /home/root/xovi-trigger ]"; then
    pass "/home/root/xovi-trigger removed"
else
    fail "/home/root/xovi-trigger still present after uninstall"
fi
...
if $SSH "! [ -f /etc/systemd/system/xovi-trigger.service ]"; then
    ...
    fail "unit file reappeared after simulated reboot, uninstall did not persist"
fi
...
log ""
log "=================================================="
log "ALL SMOKE TESTS PASSED (trigger + overlay-defeat)"
log "=================================================="
