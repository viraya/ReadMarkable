...
- USB cable (or LAN, via the wizard's "Connect by IP" button).
...
3. Writes a small systemd service (`xovi-trigger.service`) to `/etc/systemd/system/` plus a static listener binary to `/home/root/xovi-trigger/`. The listener runs at boot but does not start XOVI on its own. After each reboot, triple-tap the top of the screen (or triple-press the power button) to bring back ReadMarkable. The tablet always boots to stock software first, so a broken extension cannot bootloop the device.
