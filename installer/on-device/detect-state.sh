...
if [ -d "$xovi_path" ] && [ -x "$xovi_path/start" ]; then
    ...
    if [ -r /etc/systemd/system/xovi-trigger.service ] && \
       systemctl is-enabled xovi-trigger.service 2>/dev/null | grep -q '^enabled$'; then
...
printf '"trigger_installed":%s'  "$trigger_installed"
printf '},'
printf '"appload":{'
