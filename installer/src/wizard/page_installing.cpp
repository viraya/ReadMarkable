...
void PageInstalling::initializePage() {
    ...
    const QString flowName =
        (w->routerDecision().variant == flow::FlowVariant::Update)
    ...
    for (const QString& p : w->bundledPayload()->paths().onDeviceFiles) {
        if (p.endsWith(QStringLiteral("install-persistence.sh")) ||
            p.endsWith(QStringLiteral("xovi-activate.service")) ||
            p.endsWith(QStringLiteral("xovi-activate.sh"))) {
            persistenceFiles << p;
        }
    }
